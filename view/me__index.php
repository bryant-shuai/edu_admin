...
// print_r($_SESSION);
if( !file_exists(__PUBLIC_DIR__.''.$_SESSION['edu_user']['avatar']) ){
  $_SESSION['edu_user']['avatar'] = NULL;
}
...
?>
 ...
 <!-- onclick="call_native('event',{event_name:'PICK_IMAGE',uid:'<?=$_GET['_uid']?>',folder:'avatar',resize:100,pagemark:'<?=$pagemark?>', });" -->
 
<div onclick="call_native('event',{event_name:'PICK_IMAGE',uid:'<?=$_GET['_uid']?>',folder:'avatar',resize:100,pagemark:'<?=$pagemark?>', });" style="width:100%;height:14rem;position:relative;background:#007FFF;text-align:center;">
  <div id="user_avastar" class="s-avatar" style="width:10rem;height:10rem;border-radius: 50%;position:absolute;left:50%;top:50%;transform: translate(-50%, -50%);<?php if($_SESSION['edu_user']['avatar']){?>background-image:url(<?=$_SESSION['edu_user']['avatar']?>);<?php }?>" >
  </div>
</div>
...
window.call_from_native['UPLOAD_SUCCESS'] = function(msgstr, msg) {
  // alert('1')
  // alert('in web:'+msgstr)
  var msg = $$.str2js(msgstr)

  if( msg.pagemark != '<?=$pagemark?>' ){
    return 
  }

  var imgsrc = '<?=__HOST__?>'+msg.res.file+""
  // alert(imgsrc)
  $('#user_avastar').css("background-image",'url('+imgsrc+')')
  $$.ajax({
    url:'/me/aj_saveavatar?avatar='+msg.res.file,
    data: {
      phone: this.phone,
      password: this.password,
    },
