...
require __APP_DIR__ . '/common/CONF_TYPE.php';
...
class engine
{
    ...
    public static function run()
    {
        ...
        if (is_callable(array($cA, $action)))
        {
          $cA->$action();
        }else{
          include \view('err_500');
...
class controller
{
    ...
    public function err($code = -1, $msg = null)
    {
      $this->error($code, $msg);
    }
    ...
    {
        if(!empty($_SESSION['edu_user']) && is_array($data) ){
          $data['__s__'] = $_SESSION['edu_user'];
        } 
...
class model
{
    ...
    public static function findByIds($ids = [],$columns='*',$indexBy=null)
    {
      $ids = arrRmEmpty($ids);
      $res = self::finds('where id in ('.implode(',', $ids).')', $columns);
      if($indexBy){
    ...
    public static function findNoDeleteByIds($ids = [],$columns='*',$indexBy=null)
    {
      $ids = arrRmEmpty($ids);
      $res = self::finds('where id in ('.implode(',', $ids).') and deleted=0', $columns);
      if($indexBy){
        $res = \indexBy($res,$indexBy);
      }
      return $res;
    ...
    function setData($data,$id=NULL,$key='id'){
      $this->data = $data;
      if(!$id) $id=$data[$key];
      $this->column_value = $id;
      $this->column_key = $key;
...
class Loader
{
    public static function __autoloader($class)
    {
        ...
        // throw new \Exception("no method", 1);
        // \except(-1,'未知错误');
